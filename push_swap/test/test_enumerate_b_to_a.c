...
{
    printf("Testing EnumerateBtoA with iteration 0 data...\n");
    ...
    t_candidate *candidates = enumerate_b_to_a(enumerator, stackA, stackA_size, stackB, stackB_size, &candidate_count);
    ...
    int expected_count = stackB_size;
    if (candidate_count != expected_count) {
        printf("❌ Expected %d candidates, got %d\n", expected_count, candidate_count);
        free(candidates);
        free_candidate_enumerator(enumerator);
        return;
    }
    
    printf("✅ Got expected number of candidates: %d\n", candidate_count);
    ...
    for (int i = 0; i < candidate_count; i++) {
        ...
        if (candidate->position.from_index != i) {
            printf("❌ Candidate %d: expected FromIndex %d, got %d\n", 
                   i, i, candidate->position.from_index);
            errors++;
        }
        
        // Verify ToIndex is within valid range for stack A
        if (candidate->position.to_index < 0 || candidate->position.to_index > stackA_size) {
            printf("❌ Candidate %d: ToIndex %d is out of range [0, %d]\n", 
                   i, candidate->position.to_index, stackA_size);
            errors++;
        }
        ...
        if (candidate->score != candidate->position.total) {
            printf("❌ Candidate %d: Score %d doesn't match Total %d\n", 
    ...
    for (int i = 0; i < 5 && i < candidate_count; i++) {
        ...
        printf("  Candidate %d: FromIndex=%d, ToIndex=%d, CostA=%d, CostB=%d, Total=%d\n", 
               i, c->position.from_index, c->position.to_index, c->position.cost_a, c->position.cost_b, c->position.total);
    }
