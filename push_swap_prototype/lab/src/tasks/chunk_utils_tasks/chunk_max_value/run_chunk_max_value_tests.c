...
int run_chunk_max_value_tests(int size) {
    ...
    // Test 3: TOP_B chunk max value
    t_chunk chunk3 = {TOP_B, 3};
    int top_b_max = chunk_max_value(data, &chunk3);
    tests[2] = create_chunk_max_value_test(3, "TOP_B", data->a.stack, size, top_b_max, "TOP_B", 3);
    
    // Test 4: BOTTOM_B chunk max value
    t_chunk chunk4 = {BOTTOM_B, 4};
    int bottom_b_max = chunk_max_value(data, &chunk4);
    tests[3] = create_chunk_max_value_test(4, "BOTTOM_B", data->a.stack, size, bottom_b_max, "BOTTOM_B", 4);
