...
static t_candidate	*filter_and_select_candidates(t_candidate *candidates, 
	int candidate_count, t_selector_config config, int *result_count);
static void			cleanup_enumerate_resources(t_candidate *candidates, 
...
static t_candidate	*allocate_candidates_array(int size);
static int			*snapshot_stack_b_for_candidates(t_stack *b, int *size);
static t_candidate	create_candidate_for_node(t_candidate_enumerator *enumerator,
	t_node *node, int node_index, int *bvals, int bvals_size, int size_a);
static int			calculate_candidate_penalty(t_cost_calculator *calc, 
	int *bvals, int bvals_size, int to_idx, int val);
	...
	t_stack *a, t_stack *b, int *count)
{
	...
	if (size_a == 0)
		return (NULL);
	bvals = snapshot_stack_b_for_candidates(b, &bvals_size);
	if (!bvals)
		return (NULL);
	...
	t_node *node, int node_index, int *bvals, int bvals_size, int size_a)
{
	...
	base = position.total;
	penalty = calculate_candidate_penalty(enumerator->cost_calc, bvals, 
...
static int	calculate_candidate_penalty(t_cost_calculator *calc, 
	int *bvals, int bvals_size, int to_idx, int val)
{
	return (calculate_penalty(calc, bvals, bvals_size, to_idx, val));
}
