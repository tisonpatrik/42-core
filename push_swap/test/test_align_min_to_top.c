...
// Helper function to get top element of stack
static int get_top_element(t_stack *stack)
{
    if (!stack || !stack->head) {
        return -1; // Error value
    }
    return get_content(stack->head);
}
...
{
    printf("Running TestAlignMinToTop_Iteration0... ");
    ...
    int expectedStackASize = 43;
    int expectedStackBSize = 457;
    ...
    int topElement = get_top_element(ps->a);
    if (topElement != 1) {
        printf("❌ Expected minimum element 1 at top of stack A, got %d\n", topElement);
        test_passed = false;
    }
...
{
    printf("Running TestAlignMinToTop_Iteration1... ");
...
{
    printf("Running TestAlignMinToTop_Iteration2... ");
...
{
    printf("Running TestAlignMinToTop_SimpleCase... ");
    
    // Arrange - Set up test data where minimum is already at the top
    int stackA[] = {1, 5, 10, 15, 20};
    int stackB[] = {100, 200, 300};
    ...
    // Assert - Verify the result
    // Expected lengths from Go test: SimpleCase test
    int expectedStackASize = 5;
    int expectedStackBSize = 3;
