...
int	extract_stack_values_to_arrays(t_stack *stack, t_node **nodes,
...
void	initialize_lis_tracking_for_position(
			t_lis_computation_data *computation_data, int current_position)
{
...
void	try_extend_lis_from_previous_element(
			t_lis_computation_data *computation_data,
			int current_position, int previous_position)
{
	if (computation_data->vals[previous_position] < computation_data->vals[current_position]
		&& computation_data->lis[previous_position] + 1 > computation_data->lis[current_position])
	{
		computation_data->lis[current_position] = computation_data->lis[previous_position]
			+ 1;
		computation_data->prev[current_position] = previous_position;
	}
}


void	update_best_lis_if_improved(
			t_lis_computation_data *computation_data,
...
t_node_bool_array	*execute_lis_algorithm(t_stack *stack, int element_count)
{
	...
	t_lis_computation_data	computation_input;
	...
	if (!allocate_lis_computation_memory(&memory_allocation))
		return (NULL);
	...
	final_result = build_lis_result_from_computation(&algorithm_data,
		&computation_result);
			...
			t_lis_result *best_result)
{
	...
	{
		initialize_lis_tracking_for_position(computation_data, current_position);
		previous_position = 0;
		while (previous_position < current_position)
		{
			try_extend_lis_from_previous_element(computation_data, current_position,
				previous_position);
			previous_position++;
		}
