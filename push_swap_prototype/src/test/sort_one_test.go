...
// TestSortOneBOTTOM_A_ChunkSize1 tests sort_one function for BOTTOM_A location with chunk size 1
// Log reference: Line 826-880 - ENTRY: sort_one() with Stack A [96 97 98 99 100 94 95] and chunk size decreased by 1
func TestSortOneBOTTOM_A_ChunkSize1(t *testing.T) {
	// Arrange - EXACT data from log line 826-880
	...
	// Initial chunk state from log - chunk size 1, location BOTTOM_A
	chunk := &stack.Chunk{
		Loc:  stack.BOTTOM_A,
	...
	// Act
	solver.SortOne(ps, chunk)

	// Assert - EXACT results from log line 880
	// Expected: chunk size decreased by 1 (from 1 to 0)
	expectedSize := 0
	if chunk.Size != expectedSize {
		t.Errorf("Chunk size after SortOne = %d, want %d (from log line 880 - decreased by 1)", chunk.Size, expectedSize)
	}

	// Expected: Stack A elements changed from [96 97 98 99 100 94 95] to [95 96 97 98 99 100 94]
	// This is based on the log showing move_from_to operation and size decrease
	expectedStackA := []int{95, 96, 97, 98, 99, 100, 94}
	actualStackA := ps.A.ToSlice()

	if len(actualStackA) != len(expectedStackA) {
		t.Errorf("Stack A size after SortOne = %d, want %d (from log line 880)", len(actualStackA), len(expectedStackA))
	}

	// Check first few elements match the expected transformation from log
	for i := 0; i < len(expectedStackA) && i < len(actualStackA); i++ {
		if actualStackA[i] != expectedStackA[i] {
			t.Errorf("Stack A[%d] = %d, want %d (from log line 880)", i, actualStackA[i], expectedStackA[i])
		}
	}
}

// TestSortOneTOP_A_ChunkSize1 tests sort_one function for TOP_A location with chunk size 1
// Log reference: Line 1680 - ENTRY: sort_one() with different stack state
func TestSortOneTOP_A_ChunkSize1(t *testing.T) {
	// Arrange - EXACT data from log line 1680
	...
	// Initial chunk state - chunk size 1, location TOP_A (no move_from_to needed)
	chunk := &stack.Chunk{
		Loc:  stack.TOP_A,
		Size: 1, // EXACT from log - chunk size 1
	}
	...
	expectedSize := 0
	if chunk.Size != expectedSize {
		t.Errorf("Chunk size after SortOne = %d, want %d (from log line 1680 - decreased by 1)", chunk.Size, expectedSize)
	}
	...
	if len(actualStackA) != len(expectedStackA) {
		t.Errorf("Stack A size after SortOne = %d, want %d (from log line 1680)", len(actualStackA), len(expectedStackA))
	}

	// Check elements remain unchanged for TOP_A location
	for i := 0; i < len(expectedStackA) && i < len(actualStackA); i++ {
		if actualStackA[i] != expectedStackA[i] {
			t.Errorf("Stack A[%d] = %d, want %d (from log line 1680 - TOP_A should not change)", i, actualStackA[i], expectedStackA[i])
		}
	}
}
